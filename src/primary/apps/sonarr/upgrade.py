...
def process_cutoff_upgrades(
    app_settings: Dict[str, Any], # Changed parameters
    stop_check: Callable[[], bool] # Added stop_check
) -> bool: # Changed return type to indicate if action was taken
    ...
    processed_any = False # Track if any action was taken
    ...
    if stop_check(): sonarr_logger.info("Stop requested during upgrade processing."); return processed_any

    # Filter out already processed episodes for upgrades
    ...
    if random_upgrades:
        sonarr_logger.debug(f"Randomly selecting up to {hunt_upgrade_episodes} cutoff unmet episodes for upgrade search.")
        episodes_to_search = random.sample(episodes_to_process, min(len(episodes_to_process), hunt_upgrade_episodes))
    else:
        sonarr_logger.debug(f"Selecting the first {hunt_upgrade_episodes} cutoff unmet episodes (oldest first) for upgrade search.")
    ...
    for series_id, episode_ids in series_to_process.items():
        if stop_check(): sonarr_logger.info("Stop requested before processing next series for upgrades."); break
        ...
        if not skip_series_refresh:
            ...
            if refresh_command_id:
                ...
                if not wait_for_command(
                    ...
                    command_wait_delay, command_wait_attempts, "Series Refresh (Upgrade)", stop_check # Pass stop_check
                ):
        ...
        if stop_check(): sonarr_logger.info("Stop requested after series refresh attempt for upgrades."); break
        ...
        if search_command_id:
            ...
            if wait_for_command(
                ...
                command_wait_delay, command_wait_attempts, "Episode Upgrade Search", stop_check # Pass stop_check
            ):
    ...
    # else:
        # sonarr_logger.info("No new cutoff unmet episodes were successfully processed for upgrades in this run.") # Already logged if nothing to process
