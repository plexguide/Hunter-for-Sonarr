...
def get_configured_instances():
    ...
    instances = []
    lidarr_logger.info(f"Loaded Lidarr settings for instance check: {settings}") 
    ...
    if "instances" in settings and isinstance(settings["instances"], list) and settings["instances"]:
        lidarr_logger.info(f"Found 'instances' list with {len(settings['instances'])} items. Processing...")
        for idx, instance in enumerate(settings["instances"]):
            ...
            if is_enabled and api_url and api_key:
                ...
                lidarr_logger.info(f"Added valid instance: {instance_data}") 
            elif not is_enabled:
        ...
        lidarr_logger.info("No 'instances' list found or list is empty. Checking legacy config.")
        ...
        if api_url and api_key:
            ...
            instances.append(instance_data) 
            lidarr_logger.info(f"Added valid legacy instance: {instance_data}") 
        else:
            lidarr_logger.warning("No API URL or key found in legacy configuration")

    lidarr_logger.info(f"Returning {len(instances)} configured instances: {instances}") 
    return instances
