...
def get_queue_item_by_download_id(api_url: str, api_key: str, api_timeout: int, download_id: str) -> Optional[Dict]:
    """
    Get detailed information about a specific queue item by its download ID.
    
    Args:
        api_url: The base URL of the Radarr API
        api_key: The API key for authentication
        api_timeout: Timeout for the API request
        download_id: The download ID to search for
        
    Returns:
        The queue item dictionary if found, or None if not found or request failed
    """
    if not download_id:
        radarr_logger.warning("No download ID provided for queue lookup")
        return None
        
    # Construct the endpoint with the downloadId filter
    endpoint = f"queue?downloadId={download_id}"
    radarr_logger.debug(f"Fetching queue information for download ID: {download_id}")
    
    try:
        result = arr_request(api_url, api_key, api_timeout, endpoint)
        if result and 'records' in result and result['records']:
            # Return the first (and should be only) matching item
            queue_item = result['records'][0]
            radarr_logger.debug(f"Found queue item for download ID: {download_id}")
            return queue_item
        else:
            radarr_logger.debug(f"No queue item found for download ID: {download_id}")
            return None
    except Exception as e:
        radarr_logger.error(f"Error fetching queue item by download ID {download_id}: {e}")
        return None


def get_history_by_download_id(api_url: str, api_key: str, api_timeout: int, download_id: str) -> Optional[List[Dict]]:
    """
    Get history entries for a specific download ID.
    ...
    Returns:
        A list of history entries related to this download ID, or None if request failed
    """
    if not download_id:
        radarr_logger.warning("No download ID provided for history lookup")
        return None
        
    # Construct the endpoint with the downloadId filter
    endpoint = f"history?downloadId={download_id}&pageSize=100&sortDirection=descending&sortKey=date"
    radarr_logger.debug(f"Fetching history for download ID: {download_id}")
    
    try:
        result = arr_request(api_url, api_key, api_timeout, endpoint)
        if result and 'records' in result:
            history_items = result['records']
            radarr_logger.debug(f"Found {len(history_items)} history items for download ID: {download_id}")
            return history_items
        else:
            radarr_logger.debug(f"No history found for download ID: {download_id}")
            return []
    except Exception as e:
        radarr_logger.error(f"Error fetching history by download ID {download_id}: {e}")
        return []
